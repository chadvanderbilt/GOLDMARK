...
| `mil_code_beta/` | Minimal Python package: tiling, feature extraction, training, inference |
...
The canonical tiler is `mil_code_beta/tiling/extractor.py`. For convenience, this repo also exposes a
small CLI:

```bash
PYTHONPATH=. python -m mil_code_beta tiling data/manifests/TCGA_PTEN_slide_manifest.csv \
  --output runs \
...
```bash
PYTHONPATH=. python -m mil_code_beta features data/manifests/TCGA_PTEN_slide_manifest.csv \
